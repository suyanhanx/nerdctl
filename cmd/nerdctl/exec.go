...
func processExecCommandOptions(cmd *cobra.Command) (types.ExecCommandOptions, error) {
	globalOptions, err := processRootCmdFlags(cmd)
	if err != nil {
		return types.ExecCommandOptions{}, err
	}
	...
	if flagI {
		if flagD {
			return types.ExecCommandOptions{}, errors.New("currently flag -i and -d cannot be specified together (FIXME)")
		}
	}

	if flagT {
		if flagD {
			return types.ExecCommandOptions{}, errors.New("currently flag -t and -d cannot be specified together (FIXME)")
		}
	}
	...
	return types.ExecCommandOptions{
...
func execAction(cmd *cobra.Command, args []string) error {
	...
	return container.Exec(cmd.Context(), options, args)
}
